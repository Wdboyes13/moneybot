...
impl MoneyDatabase {
    ...
    pub fn add_money(&self, uid: u64, amount: u32) -> Result<(), MoneyError> {
        self.conn.execute(
            tblfmt!(self,"INSERT OR IGNORE INTO {} (uid, balance) VALUES (?1, ?2)"),
            params![uid, 0],
        )?;
    ...
    pub fn get_balance(&self, uid: u64) -> Result<u32, MoneyError> {
        let mut stmt = self.conn.prepare(tblfmt!(self, "SELECT uid, balance FROM {} WHERE uid = ?1"))?;
    ...
    pub fn delete_money(&self, uid: u64, amount: u32) -> Result<(), MoneyError> {
        let balance = self.get_balance(uid)?;
        if balance < amount {
            return Err(MoneyError::BalanceInsufficient{needmore: (amount - balance)});
        } else {
            self.conn.execute(
                tblfmt!(self, "UPDATE {} SET balance = ?1 WHERE uid = ?2"),
